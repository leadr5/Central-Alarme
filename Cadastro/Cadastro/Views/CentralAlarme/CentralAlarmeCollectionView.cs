...
namespace Cadastro.Views.CentralAlarmeCollectionView{
    public partial class CentralAlarmeCollectionView : XtraUserControl {
        public CentralAlarmeCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Cadastro.ViewModels.CentralAlarmeCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Cadastro.Modelo.CentralAlarme,
